...
const QuestionContent: React.FC<QuestionContentProperties> = ({
	cardCount,
	content,
}) => {
...
const GameCard: React.FC<GameCardProperties> = ({
	...
	const regularCardAnimatedStyle = useAnimatedStyle(() => {
		const spinValue = interpolate(
			...
			[NumericalValue.ZERO, NumericalValue.ONE],
			[NumericalValue.ZERO, HALF_CIRCLE_DEGREE],
		);
		const rotateValue = withTiming(`${spinValue}deg`, {
			duration: isFlipped.value ? duration : NumericalValue.ZERO,
		});
	...
	const flippedCardAnimatedStyle = useAnimatedStyle(() => {
		const spinValue = interpolate(
			Number(isFlipped.value),
			[NumericalValue.ZERO, NumericalValue.ONE],
			[HALF_CIRCLE_DEGREE, FULL_CIRCLE_DEGREE],
		);
		const rotateValue = withTiming(`${spinValue}deg`, {
			duration: isFlipped.value ? duration : NumericalValue.ZERO,
		});
